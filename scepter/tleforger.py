...
def forge_tle_single(
    ...
    argp_deg : float, optional
        Argument of perigee, in degrees. Default is 0.0.
    anomaly_deg : float, optional
    ...
    str
        A string containing three lines:
          ...
          2) TLE line 1.
          3) TLE line 2.
        Each line is constructed to be the standard TLE length (69 characters including checksum on lines 1 and 2).

    Notes
    -----
    1) Internally, mean motion is computed assuming a (near-)circular orbit of
       semi-major axis = R_earth + altitude_m. Gravity constants (GM_earth)
    ...
    2) The second derivative of mean motion (n̈/2 in SGP4) and the BSTAR
       ...
       Example outputs:
         "+12345-5" or " 12345-5" =  0.12345 × 10⁻⁵ = 1.2345e-6
         "-24500-5"              = -0.24500 × 10⁻⁵ = -2.45e-6
    ...
    ...     start_time=Time(datetime(2025, 2, 1, 12, 0, 0)),
    ...     mm_dot=0.00001,
    ...     mm_ddot=1e-5,
    ...     bstar=2.5e-5
    ... )
    >>> print(tle_str)
    TestSat
    1 00000U 25001A   25032.50000000 +.00001000  10000-4  25000-4 0    16
    2 00000  51.6000   0.0000 0000000   0.0000   0.0000 15.48892997000017
    """
...
def forge_tle_belt(
    ...
    RAAN_min: float = 0,
    RAAN_max: float = 180,
    altitude_m: float = 1200000.0,
    eccentricity: float = 0,
    inclination_deg: float = 87.9,
    argp_deg: float = 0,
    start_time: Time = Time(datetime(2025, 1, 1, 0, 0, 0)),
    mm_dot: float = 0.0,
    mm_ddot: float = 0.0,
    ...
    -------
    List[str]
        A list of three-line TLE strings, each describing one satellite.
    ...
    >>> from astropy.time import Time
    >>> from datetime import datetime
    >>> tle_list = forge_tle_belt(
    ...     belt_name="MyMegaConstellation",
    ...     num_sats_per_plane=10,
    ...     plane_count=3,
    ...     altitude_m=550000.0,
    ...     inclination_deg=53.0,
    ...     start_time=Time(datetime(2025, 3, 1, 0, 0, 0)),
    ... )
    >>> print(len(tle_list))
    30  # (10 satellites/plane * 3 planes)
    >>> print(tle_list[0])
    MyMegaConstellation_Plane_1_Satellite_1
    1 00000U 25001A   25061.00000000 ... (TLE data) ...
    2 00000  53.0000   0.0000 ...  (TLE data) ...
    """
    ...
    if belt is None:
        ...
        return np.array([
            forge_tle_single(
                ...
                argp_deg=argp_deg,
                anomaly_deg=satellite_idx * step_deg + ((step_deg / 2) if (adjacent_plane_offset and plane_idx % 2 == 1) else 0),
